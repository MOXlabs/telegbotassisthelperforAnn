...
# Данные бота
BOT_TOKEN = os.environ.get('BOT_TOKEN', '8205258292:AAEZmyuUIEk5YJLy9TEzG8rnfQCZtMdZ1DQ')
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await update.message.reply_photo(
            ...
            caption="👋 Привет, я ассистент Анны ProПРАВО. Я помогу Вам записаться на консультацию"
        )
    except Exception as e:
        logger.error(f"Ошибка отправки фото: {e}")
        await update.message.reply_text(
            "👋 Привет, я ассистент Анны ProПРАВО. Я помогу Вам записаться на консультацию"
        )
